...
extern string       gRestartMutexName           = "arduinoServerRestartMutex";
...
int WINAPI _tWinMain(HINSTANCE, HINSTANCE, LPTSTR, int)
{
	...
	try
	{
        ...
        const char appMutexName [] = "arduinoServerappMutex";
        appMutex = ::CreateMutexA(NULL, FALSE, appMutexName);
        if( ERROR_ALREADY_EXISTS == GetLastError() )
        {
             Log(lInfo) << "The ArduinoServer is already running!";
            ...
            if(gOtherAppWindow != NULL)
            {
                ...
                Log(lInfo) << "Restoring ArduinoServer!";
            }
		...
		Application->Title = "ArduinoServer";
