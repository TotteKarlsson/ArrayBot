// ucm2b2Dlg.cpp : implementation file
//
...
CUcm2b2Dlg::CUcm2b2Dlg(CWnd* pParent /*=NULL*/)
	: CDialog(CUcm2b2Dlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CUcm2b2Dlg)
...
void CUcm2b2Dlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CUcm2b2Dlg)
		// NOTE: the ClassWizard will add DDX and DDV calls here
	//}}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(CUcm2b2Dlg, CDialog)
	//{{AFX_MSG_MAP(CUcm2b2Dlg)
	ON_WM_SYSCOMMAND()
	ON_WM_DESTROY()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
...
/////////////////////////////////////////////////////////////////////////////
// CUcm2b2Dlg message handlers

BOOL CUcm2b2Dlg::OnInitDialog()
{
	...
	do
	{
		...
		usbHandle2 = USBConnectionConnect(2,DEF_MOTOR_CONTROLLER);
//		usbHandle1 = USBConnectionConnectSpec(1,DEF_MOTOR_CONTROLLER,DEF_MICROMO2PHASE);
//		usbHandle2 = USBConnectionConnectSpec(1,DEF_MOTOR_CONTROLLER,DEF_VEXTA5PHASE);
		usbHandle3 = USBConnectionConnect(1,DEF_BRIGHTLIGHT);
		usbHandle4 = USBConnectionConnect(2,DEF_BRIGHTLIGHT);
		...
		if (usbHandle2 == 0x100)
		{
			strcat(errorString,"Could not connect to motor 2\r");
			error = TRUE;
		}
		...
		if (usbHandle4 == 0x100)
		{
			strcat(errorString,"Could not connect to brightlight 2\r");
			error = TRUE;
		}
	...
	USBConnectionRead(usbHandle2,REG_SYS_PRODUCTID_SUBCLASS,&motorType2);
	switch (motorType2)
	{
	default:	// if an unknown value is returned. assume it is subclass 1;
				// the first controller version did not support this and returned 0xffffffd9
	...
	USBConnectionRead(usbHandle2,REG_USER_CURRENT_1,&temp);
	sprintf(buff,"%d",temp);
	GetDlgItem(IDC_EDIT_1_LOC1)->SetWindowText(buff);

	USBConnectionRead(usbHandle2,REG_USER_STATUS_1,&temp);
	sprintf(buff,"0x%03X",temp);
	GetDlgItem(IDC_EDIT_1_STATE1)->SetWindowText(buff);

	USBConnectionRead(usbHandle2,REG_USER_CURRENT_2,&temp);
	sprintf(buff,"%d",temp);
	GetDlgItem(IDC_EDIT_2_LOC1)->SetWindowText(buff);

	USBConnectionRead(usbHandle2,REG_USER_STATUS_2,&temp);
	sprintf(buff,"0x%03X",temp);
	GetDlgItem(IDC_EDIT_2_STATE1)->SetWindowText(buff);
	...
	USBConnectionRead(usbHandle4,REG_PWM_ABSOLUTE,&temp);
	sprintf(buff,"%5.1f",(float)(temp)/10.0);
	GetDlgItem(IDC_Value1)->SetWindowText(buff);
...
void CUcm2b2Dlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
HCURSOR CUcm2b2Dlg::OnQueryDragIcon()
{
	return (HCURSOR) m_hIcon;
}

void CUcm2b2Dlg::OnTimer(UINT nIDEvent) 
{
	temp = 0;
	USBConnectionRead(usbHandle1,REG_USER_CURRENT_1,&temp);
	sprintf(buff,"%d",temp);
...
void CUcm2b2Dlg::OnButton1Home1() 
{
	temp = 0;
	USBConnectionWrite(usbHandle1, REG_USER_LIMIT_1, &temp);
}

void CUcm2b2Dlg::OnButton1Minus1() 
{
	temp = -100;
	USBConnectionWrite(usbHandle1, REG_USER_INCREMENT_1, &temp);
}

void CUcm2b2Dlg::OnButton11000g1() 
{
	temp = 1000;
	USBConnectionWrite(usbHandle1, REG_USER_TARGET_1, &temp);
}

void CUcm2b2Dlg::OnButton1Plus1() 
{
	temp = +100;
	USBConnectionWrite(usbHandle1, REG_USER_INCREMENT_1, &temp);
}

void CUcm2b2Dlg::OnButton1Limit1() 
{
	temp = 1;
	USBConnectionWrite(usbHandle1, REG_USER_LIMIT_1, &temp);
}

void CUcm2b2Dlg::OnButton2Home1() 
{
	temp = 0;
	USBConnectionWrite(usbHandle1, REG_USER_LIMIT_2, &temp);
}

void CUcm2b2Dlg::OnButton2Minus1() 
{
	temp = -100;
	USBConnectionWrite(usbHandle1, REG_USER_INCREMENT_2, &temp);
}

void CUcm2b2Dlg::OnButton21000g1() 
{
	temp = 1000;
	USBConnectionWrite(usbHandle1, REG_USER_TARGET_2, &temp);
}

void CUcm2b2Dlg::OnButton2Plus1() 
{
	temp = +100;
	USBConnectionWrite(usbHandle1, REG_USER_INCREMENT_2, &temp);
}

void CUcm2b2Dlg::OnButton2Limit1() 
{
	temp = 1;
	USBConnectionWrite(usbHandle1, REG_USER_LIMIT_2, &temp);
}

void CUcm2b2Dlg::OnButton1Home2() 
{
	temp = 0;
	USBConnectionWrite(usbHandle2, REG_USER_LIMIT_1, &temp);
}

void CUcm2b2Dlg::OnButton1Minus2() 
{
	temp = -100;
	USBConnectionWrite(usbHandle2, REG_USER_INCREMENT_1, &temp);
}

void CUcm2b2Dlg::OnButton11000g2() 
{
	temp = 1000;
	USBConnectionWrite(usbHandle2, REG_USER_TARGET_1, &temp);
}

void CUcm2b2Dlg::OnButton1Plus2() 
{
	temp = +100;
	USBConnectionWrite(usbHandle2, REG_USER_INCREMENT_1, &temp);
}

void CUcm2b2Dlg::OnButton1Limit2() 
{
	temp = 1;
	USBConnectionWrite(usbHandle2, REG_USER_LIMIT_1, &temp);
}

void CUcm2b2Dlg::OnButton2Home2() 
{
	temp = 0;
	USBConnectionWrite(usbHandle2, REG_USER_LIMIT_2, &temp);
}

void CUcm2b2Dlg::OnButton2Minus2() 
{
	temp = -100;
	USBConnectionWrite(usbHandle2, REG_USER_INCREMENT_2, &temp);
}

void CUcm2b2Dlg::OnButton2100g2() 
{
	temp = 1000;
	USBConnectionWrite(usbHandle2, REG_USER_TARGET_2, &temp);
}

void CUcm2b2Dlg::OnButton2Plus2() 
{
	temp = +100;
	USBConnectionWrite(usbHandle2, REG_USER_INCREMENT_2, &temp);
}

void CUcm2b2Dlg::OnButton2Limit2() 
{
	temp = 1;
	USBConnectionWrite(usbHandle2, REG_USER_LIMIT_2, &temp);
}

void CUcm2b2Dlg::OnButtonOff1() 
{
	temp = 0;
	USBConnectionWrite(usbHandle3,REG_PWM_ABSOLUTE,&temp);
}

void CUcm2b2Dlg::OnButtonMinus1() 
{
	temp = -100;
	USBConnectionWrite(usbHandle3,REG_PWM_INCREMENT,&temp);
}

void CUcm2b2Dlg::OnButtonMid1() 
{
	temp = 500;
	USBConnectionWrite(usbHandle3,REG_PWM_ABSOLUTE,&temp);
}

void CUcm2b2Dlg::OnButtonPlus1() 
{
	temp = +100;
	USBConnectionWrite(usbHandle3,REG_PWM_INCREMENT,&temp);
}

void CUcm2b2Dlg::OnButtonFullon1() 
{
	temp = 1000;
	USBConnectionWrite(usbHandle3,REG_PWM_ABSOLUTE,&temp);
}

void CUcm2b2Dlg::OnButtonOff2() 
{
	temp = 0;
	USBConnectionWrite(usbHandle4,REG_PWM_ABSOLUTE,&temp);
}

void CUcm2b2Dlg::OnButtonMinus2() 
{
	temp = -100;
	USBConnectionWrite(usbHandle4,REG_PWM_INCREMENT,&temp);
}

void CUcm2b2Dlg::OnButtonMid2() 
{
	temp = 500;
	USBConnectionWrite(usbHandle4,REG_PWM_ABSOLUTE,&temp);
}

void CUcm2b2Dlg::OnButtonPlus2() 
{
	temp = +100;
	USBConnectionWrite(usbHandle4,REG_PWM_INCREMENT,&temp);
}

void CUcm2b2Dlg::OnButtonFullon2() 
{
	temp = 1000;
	USBConnectionWrite(usbHandle4,REG_PWM_ABSOLUTE,&temp);
}
